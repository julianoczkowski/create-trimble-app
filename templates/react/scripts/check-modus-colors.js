...
const TAILWIND_COLOR_PATTERNS = [
  ...
  // Only flag CSS variables that contain "color" in the name
  /var\(--modus-wc-color-[^)]*\)/g,
  // Exclude non-color CSS variables (line-height, spacing, font-size, etc.)
  // This pattern is intentionally commented out as it's too broad
  // /var\(--(?!border|ring|input|radius|line-height|spacing|font-size|font-weight|z-index|opacity|shadow|transition|animation)[^)]*\)/g,
];
...
const EXCLUDE_PATTERNS = [
  ...
  "src/App.css", // Exclude App.css as it's not used (unused Vite template file)
