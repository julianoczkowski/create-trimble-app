...
SolidJS 1.9.5 + Vite boilerplate with Modus 2 Web Components integration (Trimble Design System). Uses TypeScript, Tailwind CSS 3, and @solidjs/router.
...
Add your routes in `src/config/routes.ts` and `src/App.tsx`.

### SolidJS Patterns
- Use `createSignal` instead of `useState` for reactive state
- Use `createEffect`, `onMount`, `onCleanup` instead of `useEffect` for side effects
- Use `@solidjs/router` for routing (not react-router-dom)
- Use `@trimble-oss/moduswebcomponents` (vanilla) - no React wrapper package
- Use `on:eventName` for custom events on web components (e.g., `on:inputChange`)
- Use `attr:propName` for attribute binding when needed
- Let Modus components manage their own state (don't control accordion/modal state from SolidJS)

### Component Pattern
- SolidJS wrapper components in `src/components/` wrap `@trimble-oss/moduswebcomponents` web components
...
Use ref pattern with imperative handle. Access dialog via `querySelector("dialog")` for `showModal()`/`close()`.
...
- **@solidjs/testing-library** for SolidJS component testing
