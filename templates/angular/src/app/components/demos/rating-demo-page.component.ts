...
import type { IRatingChange } from '@trimble-oss/moduswebcomponents';
  ...
  template: `
    ...
    >
      ...
      <demo-example
        ...
        description="Rating with two-way binding and event handling."
      >
        <div class="flex flex-col gap-6">
          <modus-rating
            variant="star"
          ...
          <div class="p-4 rounded-lg bg-card border-default">
            <p class="text-sm text-foreground mb-2">
              <strong>Current Rating:</strong> {{ ratingValue() }}
            </p>
            <p class="text-sm text-muted-foreground">
              Click or hover over the stars above to change the rating.
            </p>
          </div>
        </div>
      </demo-example>
...
export class RatingDemoPageComponent {
  readonly ratingValue = signal<number>(3.5);

  handleRatingChange(change: IRatingChange): void {
    this.ratingValue.set(change.newRating);
  }
}
